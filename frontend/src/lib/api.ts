...
export async function listObjects(prefix = ''): Promise<S3Object[]> {
    const response = await fetch(`${BASE_URL}/objects?prefix=${encodeURIComponent(prefix)}`);
    if (!response.ok) {
...
export async function viewFile(path: string): Promise<string> {
    const response = await fetch(`${BASE_URL}/files/view?path=${encodeURIComponent(path)}`);
    if (!response.ok) {
...
export async function updateFile(path: string, content: string): Promise<void> {
    const response = await fetch(`${BASE_URL}/files/update`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
export async function renameObject(oldPath: string, newPath: string): Promise<void> {
    const response = await fetch(`${BASE_URL}/objects/rename`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
